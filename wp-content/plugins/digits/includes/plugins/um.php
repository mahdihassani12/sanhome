...
add_action('admin_notices', 'digits_um_migration_notice');
function digits_um_migration_notice()
{

    if (isset($_POST['dig_hid_um_migration_notice']) || !class_exists('UM')) {
        update_option('dig_hid_um_migration_notice', 1);
        return;
    }
    if (get_option('dig_hid_um_migration_notice', -1) == -1) {
        ?>
        <div class="notice notice-error dig-new-activation-notice is-dismissible" style="display: block">
            <p>
                <b>DIGITS NOTICE: </b>
                We've made functionality change for Ultimate Membership Forms, please make sure to set Mobile Number
                field
                as Required if you want it to be compulsory. <a
                        href="https://help.unitedover.com/digits/kb/how-to-use-digits-with-ultimate-member-forms/"
                        target="_blank">Visit here</a> for documentation (Point 4).
            </p>

            <form method="post">
                <input type="hidden" name="dig_hid_um_migration_notice" value="1"/>
