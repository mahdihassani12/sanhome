...
$rhea_get_temr_count = (count($features_terms));
...
<?php
	...
	foreach ( $features_terms as $fet_terms ) {
	    if($counter < $max_limit) {
		    ?>
            <li class="rhea_feature_item">
                <span class="rhea_check_icon"><?php include RHEA_ASSETS_DIR . '/icons/check.svg'; ?></span>
                <a href="<?php echo esc_attr( get_term_link( $fet_terms->slug, 'property-feature' ) ); ?>">
				    <?php echo esc_html( $fet_terms->name ); ?>
                </a>
            </li>
		    <?php
	    }
	...
	if($rhea_get_temr_count > $max_limit){
	?>
        <li class="rhea_fp_features_count">
            <?php
                $terms_left = $rhea_get_temr_count - $max_limit;
            ?>
