...
if ( ! function_exists( 'ere_register_property_type_taxonomy' ) ) :
	function ere_register_property_type_taxonomy() {
		...
		$type_labels = array(
			...
			'singular_name'              => esc_html__( 'نوعیت ملک', 'easy-real-estate' ),
...
if ( ! function_exists( 'ere_property_rest_api_meta_field' ) ) {
	...
	function ere_property_rest_api_meta_field() {
		register_rest_field( 'property', 'property_meta', array(
			...
			'update_callback' => 'ere_update_property_meta_for_rest_api'
			
		) );
	}
	
	function ere_update_property_meta_for_rest_api($object, $meta_value){
		
		
		$post_id = $meta_value->ID;
		
		//for($i = 0;$i < count($object) ; $i++){}}
		
		foreach($object as $key => $value){
				
			$havemetafield  = get_post_meta($post_id, $key, false);
			if ($havemetafield) {
					$ret = update_post_meta($post_id, $key, $value );
			} else {
					$ret = add_post_meta( $post_id, $key, $value  );
			}
