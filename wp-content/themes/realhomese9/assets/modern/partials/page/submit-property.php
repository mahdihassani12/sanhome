...
function update_property_meta_data($id, $value,$meta ,$single = true) {
    $havemetafield  = get_post_meta($id,$meta, false);
    var_dump("start");
    var_dump($havemetafield);
    if ($havemetafield) {
        $ret = update_post_meta($id, $meta,$value );
        var_dump("update is:".$ret);
    } else {
        $ret = add_post_meta( $id, $meta, $value ,$single );
        var_dump("insert is:".$ret);
    ...
    if (wp_verify_nonce($_POST['property_nonce'], 'submit_property')) {
        ...
        if ($property_id > 0) {
            ...
            if (isset($_POST['agent_display_option']) && !empty($_POST['agent_display_option'])) {
                ...
                switch ($_POST['agent_display_option']) {

                    case "normal":

                        $normal_count = get_user_meta(get_current_user_id(),"normal_properties",true);
                        update_user_meta(get_current_user_id(),"normal_properties",--$normal_count);
                        update_post_meta($property_id, 'REAL_HOMES_property_score', 30);

                        break;
                    case "vip":

                        $normal_count = get_user_meta(get_current_user_id(),"vip_properties",true);
                        update_user_meta(get_current_user_id(),"vip_properties",--$normal_count);
                        update_post_meta($property_id, 'REAL_HOMES_property_score', 90);

                        break;
                    case "golden":
                        $normal_count = get_user_meta(get_current_user_id(),"golden_properties",true);
                        update_user_meta(get_current_user_id(),"golden_properties",--$normal_count);
                        update_post_meta($property_id, 'REAL_HOMES_property_score', 150);
...
//                }
            ...
            if (isset($_POST['fullname']) && !empty($_POST['fullname'])) {
                update_post_meta($property_id,'inspiry_fullname',$_POST['fullname']);
//                update_post_meta(, '', ());
            }
            if (isset($_POST['mobile']) && !empty($_POST['mobile'])) {
                update_post_meta($property_id, 'inspiry_mobile', ($_POST['mobile']));
            }
            if (isset($_POST['whatsup']) && !empty($_POST['whatsup'])) {
                update_post_meta($property_id, 'inspiry_whatsup', ($_POST['whatsup']));
            }
                ...
                <?php if (empty($header_variation) || ('none' === $header_variation)) : ?>
                    <h2 class="rh_page__title">
                        ...
                        if (inspiry_is_edit_property()) :
                            ?>
                            <span class="sub"><?php esc_html_e('Edit', 'easy-real-estate'); ?></span>
                            <span class="title"><?php esc_html_e('Property', 'easy-real-estate'); ?></span>
                        <?php
                        else :
                            // Page Title.
                ...
                <div class="rh_page__nav">
                    ...
                    if (!empty($profile_url)) :
                        ...
                        <a href="<?php echo esc_url($profile_url); ?>" class="rh_page__nav_item">
                            ...
                            <p><?php esc_html_e('Profile', 'easy-real-estate'); ?></p>
                        </a>
                    <?php
                    endif;
                    ...
                    if (!empty($my_properties_url)) :
                        ...
                        <a href="<?php echo esc_url($my_properties_url); ?>" class="rh_page__nav_item">
                            ...
                            <p><?php esc_html_e('My Properties', 'easy-real-estate'); ?></p>
                        </a>
                    <?php
                    endif;
                    ...
                    if (!empty($favorites_url)) :
                        ...
                        <a href="<?php echo esc_url($favorites_url); ?>" class="rh_page__nav_item">
                            ...
                            <p><?php esc_html_e('Favorites', 'easy-real-estate'); ?></p>
                        </a>
                    <?php endif; ?>
            ...
            if (!is_user_logged_in()) {
                ...
                if (!empty($enable_user_nav) && 'true' === $enable_user_nav) {
                    if (empty($theme_login_url)) {
                        alert(esc_html__('Login Required:', 'easy-real-estate'), esc_html__('Please login to submit property!', 'easy-real-estate'));
                    } elseif (!empty($theme_login_url)) {
                        alert(esc_html__('Login Required:', 'easy-real-estate'), sprintf(esc_html__('Please %1$s login %2$s to submit property!', 'easy-real-estate'), '<a href="' . esc_url($theme_login_url) . '">', '</a>'));
                    }
                } else {
                    alert(esc_html__('Login Required:', 'easy-real-estate'), esc_html__('Please login to submit property!', 'easy-real-estate'));
                }
                ...
                if ($invalid_nonce) {
                    ...
                    alert(esc_html__('Error:', 'easy-real-estate'), esc_html__('Security check failed!', 'easy-real-estate'));

                } else {

                    if ($submitted_successfully) {

                        alert(esc_html__('Success:', 'easy-real-estate'), get_option('theme_submit_message'));

                    } elseif ($updated_successfully) {

                        alert(esc_html__('Success:', 'easy-real-estate'), esc_html__('Property updated successfully!', 'easy-real-estate'));
