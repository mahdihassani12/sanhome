(function ($) {
    ...
    function showAndHide(className){
        ...
        console.log($('.features-checkboxes > li').hasClass(className));
        $('.features-checkboxes li').each(function(){
    ...
    $(document).ready(function () {
        ...
        var locationInput = $(".location-selected");
        var locationShow = $(".selected-location-show");

        var loading_content = $(".location-list").html();


        var ajaxurl = locationInput.attr("action");


        locationList.hide();
        locationShow.hide();
        $(document).on('click', '.select-location-items', function (e) {

            e.preventDefault();
            // alert("ckilded")
            var term_id = $(this).attr("term_id");
            var main_term = $(this).attr("main-term");
            
            locationShow.find(".selected-location-show-input").text(main_term);
            locationInput.val(term_id);
            locationList.html("");
            locationList.hide();
        ...
        $(document).on('click', '.remove-address', function (e) {
            ...
            e.preventDefault();
            locationShow.find(".selected-location-show-input").text("");
            locationInput.val("");
            locationShow.hide();
        ...
        locationInput.keyup(function () {
            ...
            request = $.ajax({
                ...
                type: "GET",
                beforeSend: function () {
                    locationList.html("<div class='d-flex'><p class='text-center loading-locations'>صبر کنید...</p></div>");
                    if (request !== null) {
                        request.abort();
                    }
                },
                    ...
                    if (response.success) {
                        ...
                        if (response.data.length > 0) {
                            response.data.forEach(function (index) {
                                ...
                                html += "<div term_id='" + index.term_taxonomy_id + "' main-term='"+index.term_name+"' class='select-location-items d-flex'> " +
                                    "<div class='icon-location'><i class='fa fa-location-arrow'></i></div>" +
                                    "<div class='items-location d-flex column-1'>" +
                                    "<div class='main-term'>" + index.term_name + "</div>" +
                                    "<div class='parent-term'>" + index.term_parent + "</div>"
                                    +
                                    "</div>" +
                            ...
                            html = 'جای مورد نظرتان موجود نیست';
                        }
        ...
        $('#type').change(function () {
            ...
            var currentSelect =  $(this).find('option:selected').attr('type-property');

            console.log(currentSelect);

            showAndHide(currentSelect);

        })



        $(".select-location-items").click(function () {

            alert("ckilded")
            // var term_id = $(this).attr("term_id");
            // var main_term = $(this).attr("main-term");
            //
            // locationShow.text(main_term);
            // locationInput.val(term_id);
        ...
        window.onclick = function (event) {
            // console.log(event);
            if (event.target == "div#package-form-modal") {
                $("#package-form-modal").hide();
            }
        }


        $("#package_form_submit").on('submit', function (event) {
            ...
            $(this).find("#form-button").val("در حال ارسال...");
            $(this).find("#form-button").prop("disabled", true);
            data += "&action=add_front_package_user";
        ...
        $(".elementor-price-table__button").click(function (event) {
            ...
            switch (link) {
                ...
                case "#package2":
                    ...
                    $("#golden-input").val("6");
                    $("#vip-input").val("20");
                    $("#normal-input").val('800');

                    break;
                case "#package3":
                    $("#golden-input").val("15");
                    $("#vip-input").val("50");
                    $("#normal-input").val('300');

                    break;
                case "#package4":
                    $("#golden-input").val("40");
                    $("#vip-input").val("100");
                    $("#normal-input").val('600');
                    break;
                case "#package5":
                    $("#golden-input").val("100");
                    $("#vip-input").val("200");
                    $("#normal-input").val('1200');
                    break;
                case "#package6":
                    $("#golden-input").val("150");
                    $("#vip-input").val("300");
                    $("#normal-input").val('2000');
                    break;

            }
            $(".package-selected").val(link);
