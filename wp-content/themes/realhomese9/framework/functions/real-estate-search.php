		...
		if ( ! empty( $taxonomy_terms ) ) {
			foreach ( $taxonomy_terms as $term ) {
				...
				if ( $target_term_id == $term->term_id ) {
					echo '<option  type-property="'.$term->slug.'" value="' . $term->term_id . '" selected="selected">' . $prefix . $term->name . '</option>';
				} else {
					echo '<option type-property="'.$term->slug.'" value="' . $term->term_id . '">' . $prefix . $term->name . '</option>';
				}
...
if ( !function_exists( 'inspiry_get_location_titles' ) ) :
	...
	function inspiry_get_location_titles() {
		...
		$location_select_titles = array(
			__( 'موقعیت اصلی', 'framework' ),
			__( 'موقعیت فرعی', 'framework' ),
