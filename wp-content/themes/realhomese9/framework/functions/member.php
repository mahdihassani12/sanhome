...
if ( ! function_exists( 'inspiry_ajax_login' ) ) :
	...
	function inspiry_ajax_login() {
		...
		inspiry_auth_user_login( $_POST['log'], $_POST['pwd'], esc_html__( 'Login', 'easy-real-estate' ) );
	...
	function inspiry_auth_user_login( $user_login, $password, $login ) {
		...
		if ( is_wp_error( $user_signon ) ) {
			echo json_encode( array(
				'success' => false,
				'message' => esc_html__( '* Wrong username or password.', 'easy-real-estate' ),
			) );
		} else {
			wp_set_current_user( $user_signon->ID );
			echo json_encode( array(
				'success' => true,
				'message' => $login . ' ' . esc_html__( 'successful. Redirecting...', 'easy-real-estate' ),
	...
	function inspiry_ajax_register() {
		...
		if ( is_wp_error( $user_register ) ) {
			...
			if ( in_array( 'empty_user_login', $error ) ) {
				...
				echo json_encode( array(
					'success' => false,
					'message' => esc_html__( 'نام کاربری قبلا موجود است.', 'easy-real-estate' )
				) );
			} elseif ( in_array( 'existing_user_email', $error ) ) {
				echo json_encode( array(
					'success' => false,
					'message' => esc_html__( 'این ایمیل قبلا ثبت شده.', 'easy-real-estate' )
				) );
			} else {
				echo json_encode( array(
					'success' => false,
			...
			echo json_encode( array(
				'success' => true,
				'message' => esc_html__( 'شما موفقانه ثبت شده اید. لطفا ایمیل خود را چک کنید.', 'easy-real-estate' ),
			) );
